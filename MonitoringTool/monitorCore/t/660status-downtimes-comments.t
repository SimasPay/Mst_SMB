...
while($iteration <= $iterations_max) {
	...
	my $start = time;
	$output = `NAGIOS_CGI_CONFIG=etc/cgi-with-generated-status.cfg REQUEST_METHOD=GET REMOTE_USER=monitoradmin QUERY_STRING="host=host1" $status_cgi`;
	...
	#$output = `NAGIOS_CGI_CONFIG=etc/cgi-with-generated-status.cfg REQUEST_METHOD=GET REMOTE_USER=monitoradmin QUERY_STRING="type=2&host=host1&service=Dummy+service" $extinfo_cgi`;
