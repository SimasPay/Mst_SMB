...
void request_command_data(int cmd) {
	...
	switch(cmd) {
		...
		case CMD_RESTART_PROCESS:
			printf("%s the Monitor process", (cmd == CMD_SHUTDOWN_PROCESS) ? "shutdown" : "restart");
			break;
...
void commit_command_data(int cmd) {
	...
	if(use_authentication == FALSE) {
		if(content_type == WML_CONTENT)
			...
			printf("I don't want to be personally responsible for what may happen as a result of allowing unauthorized users to issue commands to Monitor,");
	...
	/* if Monitor isn't checking external commands, don't do anything... */
	else if(check_external_commands == FALSE) {
		if(content_type == WML_CONTENT)
			printf("<p>Error: Monitor is not checking external commands!</p>\n");
		else {
			printf("<P><DIV CLASS='errorMessage'>Sorry, but Monitor is currently not checking for external commands, so your command will not be committed!</DIV></P>\n");
	...
	/* everything looks okay, so let's go ahead and commit the command... */
		...
		if(result == OK) {
			if(content_type == WML_CONTENT)
				...
				printf("<P><DIV CLASS='infoMessage'>Your command request was successfully submitted to Monitor for processing.<BR><BR>\n");
...
int write_command_to_file(char *cmd) {
	...
	if(stat(command_file, &statbuf)) {
		...
		if(content_type == WML_CONTENT)
			...
			printf("The external command file may be missing, Monitor may not be running, and/or Monitor may not be checking external commands.\n");
			printf("</DIV></P>\n");
			}
...
void show_command_help(cmd) {
	...
	switch(cmd) {
		...
		case CMD_ADD_HOST_COMMENT:
			...
			printf("the next time Monitor is restarted.\n");
			break;
		...
		case CMD_ADD_SVC_COMMENT:
			...
			printf("deleted the next time Monitor is restarted.\n");
			break;
		...
		case CMD_SCHEDULE_SVC_CHECK:
			printf("This command is used to schedule the next check of a particular service.  Monitor will re-queue the service to be checked at the time you specify.\n");
			printf("If you select the <i>force check</i> option, Monitor will force a check of the service regardless of both what time the scheduled check occurs and whether or not checks are enabled for the service.\n");
			break;
		...
		case CMD_SHUTDOWN_PROCESS:
			printf("This command is used to shutdown the Monitor process. Note: Once the Monitor has been shutdown, it cannot be restarted via the web interface!\n");
			break;

		case CMD_RESTART_PROCESS:
			printf("This command is used to restart the Monitor process.   Executing a restart command is equivalent to sending the process a HUP signal.\n");
			printf("All information will be flushed from memory, the configuration files will be re-read, and Monitor will start monitoring with the new configuration information.\n");
			break;
		...
		case CMD_DISABLE_HOST_SVC_CHECKS:
			printf("This command is used to disable active checks of all services associated with the specified host.  When a service is disabled Monitor will not monitor the service.  Doing this will prevent any notifications being sent out for\n");
			printf("the specified service while it is disabled.  In order to have Monitor check the service in the future you will have to re-enable the service.\n");
		...
		case CMD_STOP_EXECUTING_SVC_CHECKS:
			printf("This command is used to temporarily stop Monitor from actively executing any service checks.  This will have the side effect of preventing any notifications from being sent out (for any and all services and hosts).\n");
			printf("Service checks will not be executed again until you issue a command to resume service check execution.\n");
			break;

		case CMD_START_ACCEPTING_PASSIVE_SVC_CHECKS:
			printf("This command is used to make Monitor start accepting passive service check results that it finds in the external command file\n");
			break;

		case CMD_STOP_ACCEPTING_PASSIVE_SVC_CHECKS:
			printf("This command is use to make Monitor stop accepting passive service check results that it finds in the external command file.  All passive check results that are found will be ignored.\n");
			break;

		case CMD_ENABLE_PASSIVE_SVC_CHECKS:
			printf("This command is used to allow Monitor to accept passive service check results that it finds in the external command file for this particular service.\n");
			break;

		case CMD_DISABLE_PASSIVE_SVC_CHECKS:
			printf("This command is used to stop Monitor accepting passive service check results that it finds in the external command file for this particular service.  All passive check results that are found for this service will be ignored.\n");
			break;

		case CMD_ENABLE_EVENT_HANDLERS:
			printf("This command is used to allow Monitor to run host and service event handlers.\n");
			break;

		case CMD_DISABLE_EVENT_HANDLERS:
			printf("This command is used to temporarily prevent Monitor from running any host or service event handlers.\n");
			break;

		case CMD_ENABLE_SVC_EVENT_HANDLER:
			printf("This command is used to allow Monitor to run the service event handler for a particular service when necessary (if one is defined).\n");
			break;

		case CMD_DISABLE_SVC_EVENT_HANDLER:
			printf("This command is used to temporarily prevent Monitor from running the service event handler for a particular service.\n");
			break;

		case CMD_ENABLE_HOST_EVENT_HANDLER:
			printf("This command is used to allow Monitor to run the host event handler for a particular service when necessary (if one is defined).\n");
			break;

		case CMD_DISABLE_HOST_EVENT_HANDLER:
			printf("This command is used to temporarily prevent Monitor from running the host event handler for a particular host.\n");
			break;
		...
		case CMD_DISABLE_HOST_CHECK:
			printf("This command is used to temporarily prevent Monitor from actively checking the status of a particular host.  If Monitor needs to check the status of this host, it will assume that it is in the same state that it was in before checks were disabled.\n");
			break;

		case CMD_START_OBSESSING_OVER_SVC_CHECKS:
			printf("This command is used to have Monitor start obsessing over service checks.  Read the documentation on distributed monitoring for more information on this.\n");
			break;

		case CMD_STOP_OBSESSING_OVER_SVC_CHECKS:
			printf("This command is used stop Monitor from obsessing over service checks.\n");
			break;
		...
		case CMD_SCHEDULE_HOST_DOWNTIME:
			printf("This command is used to schedule downtime for a particular host.  During the specified downtime, Monitor will not send notifications out about the host.\n");
			printf("When the scheduled downtime expires, Monitor will send out notifications for this host as it normally would.  Scheduled downtimes are preserved\n");
			...
			printf("option, Monitor will treat this as \"flexible\" downtime.  Flexible downtime starts when the host goes down or becomes unreachable (sometime between the\n");
		...
		case CMD_SCHEDULE_HOST_SVC_DOWNTIME:
			printf("This command is used to schedule downtime for all services on a particular host.  During the specified downtime, Monitor will not send notifications out about the host.\n");
		...
		case CMD_SCHEDULE_SVC_DOWNTIME:
			printf("This command is used to schedule downtime for a particular service.  During the specified downtime, Monitor will not send notifications out about the service.\n");
			printf("When the scheduled downtime expires, Monitor will send out notifications for this service as it normally would.  Scheduled downtimes are preserved\n");
			printf("across program shutdowns and restarts.  Both the start and end times should be specified in the following format:  <b>mm/dd/yyyy hh:mm:ss</b>.\n");
			printf("If you select the <i>fixed</i> option, the downtime will be in effect between the start and end times you specify.  If you do not select the <i>fixed</i>\n");
			printf("option, Monitor will treat this as \"flexible\" downtime.  Flexible downtime starts when the service enters a non-OK state (sometime between the\n");
			printf("start and end times you specified) and lasts as long as the duration of time you enter.  The duration fields do not apply for fixed downtime.\n");
			break;
		...
		case CMD_SCHEDULE_HOSTGROUP_HOST_DOWNTIME:
			printf("This command is used to schedule downtime for all hosts in a particular hostgroup.  During the specified downtime, Monitor will not send notifications out about the hosts.\n");
			printf("When the scheduled downtime expires, Monitor will send out notifications for the hosts as it normally would.  Scheduled downtimes are preserved\n");
			printf("across program shutdowns and restarts.  Both the start and end times should be specified in the following format:  <b>mm/dd/yyyy hh:mm:ss</b>.\n");
			printf("If you select the <i>fixed</i> option, the downtime will be in effect between the start and end times you specify.  If you do not select the <i>fixed</i>\n");
			printf("option, Monitor will treat this as \"flexible\" downtime.  Flexible downtime starts when a host goes down or becomes unreachable (sometime between the\n");
		...
		case CMD_SCHEDULE_HOSTGROUP_SVC_DOWNTIME:
			printf("This command is used to schedule downtime for all services in a particular hostgroup.  During the specified downtime, Monitor will not send notifications out about the services.\n");
			printf("When the scheduled downtime expires, Monitor will send out notifications for the services as it normally would.  Scheduled downtimes are preserved\n");
			printf("across program shutdowns and restarts.  Both the start and end times should be specified in the following format:  <b>mm/dd/yyyy hh:mm:ss</b>.\n");
			printf("If you select the <i>fixed</i> option, the downtime will be in effect between the start and end times you specify.  If you do not select the <i>fixed</i>\n");
			printf("option, Monitor will treat this as \"flexible\" downtime.  Flexible downtime starts when a service enters a non-OK state (sometime between the\n");
			printf("start and end times you specified) and lasts as long as the duration of time you enter.  The duration fields do not apply for fixed dowtime.\n");
		...
		case CMD_START_ACCEPTING_PASSIVE_HOST_CHECKS:
			printf("This command is used to have Monitor start obsessing over host checks.  Read the documentation on distributed monitoring for more information on this.\n");
			break;

		case CMD_STOP_ACCEPTING_PASSIVE_HOST_CHECKS:
			printf("This command is used to stop Monitor from obsessing over host checks.\n");
			break;

		case CMD_ENABLE_PASSIVE_HOST_CHECKS:
			printf("This command is used to allow Monitor to accept passive host check results that it finds in the external command file for a particular host.\n");
			break;

		case CMD_DISABLE_PASSIVE_HOST_CHECKS:
			printf("This command is used to stop Monitor from accepting passive host check results that it finds in the external command file for a particular host.  All passive check results that are found for this host will be ignored.\n");
			break;
		...
		case CMD_SCHEDULE_HOST_CHECK:
			printf("This command is used to schedule the next check of a particular host.  Monitor will re-queue the host to be checked at the time you specify.\n");
			printf("If you select the <i>force check</i> option, Monitor will force a check of the host regardless of both what time the scheduled check occurs and whether or not checks are enabled for the host.\n");
			break;

		case CMD_START_OBSESSING_OVER_SVC:
			printf("This command is used to have Monitor start obsessing over a particular service.\n");
			break;

		case CMD_STOP_OBSESSING_OVER_SVC:
			printf("This command is used to stop Monitor from obsessing over a particular service.\n");
			break;

		case CMD_START_OBSESSING_OVER_HOST:
			printf("This command is used to have Monitor start obsessing over a particular host.\n");
			break;

		case CMD_STOP_OBSESSING_OVER_HOST:
			printf("This command is used to stop Monitor from obsessing over a particular host.\n");
			break;
		...
		case CMD_SCHEDULE_SERVICEGROUP_HOST_DOWNTIME:
			printf("This command is used to schedule downtime for all hosts in a particular servicegroup.  During the specified downtime, Monitor will not send notifications out about the hosts.\n");
			printf("When the scheduled downtime expires, Monitor will send out notifications for the hosts as it normally would.  Scheduled downtimes are preserved\n");
			printf("across program shutdowns and restarts.  Both the start and end times should be specified in the following format:  <b>mm/dd/yyyy hh:mm:ss</b>.\n");
			printf("If you select the <i>fixed</i> option, the downtime will be in effect between the start and end times you specify.  If you do not select the <i>fixed</i>\n");
			printf("option, Monitor will treat this as \"flexible\" downtime.  Flexible downtime starts when a host goes down or becomes unreachable (sometime between the\n");
		...
		case CMD_SCHEDULE_SERVICEGROUP_SVC_DOWNTIME:
			printf("This command is used to schedule downtime for all services in a particular servicegroup.  During the specified downtime, Monitor will not send notifications out about the services.\n");
			printf("When the scheduled downtime expires, Monitor will send out notifications for the services as it normally would.  Scheduled downtimes are preserved\n");
			printf("across program shutdowns and restarts.  Both the start and end times should be specified in the following format:  <b>mm/dd/yyyy hh:mm:ss</b>.\n");
			printf("If you select the <i>fixed</i> option, the downtime will be in effect between the start and end times you specify.  If you do not select the <i>fixed</i>\n");
			printf("option, Monitor will treat this as \"flexible\" downtime.  Flexible downtime starts when a service enters a non-OK state (sometime between the\n");
