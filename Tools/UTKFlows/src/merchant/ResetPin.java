...
public class ResetPin implements CommandListener {
    ...
    public ResetPin(MobileBankingMidlet m, Form pf) {
        parent = m;
        parentForm = pf;
        display = Display.getDisplay(parent);
        f = new Form("Merchant Reset Pin");

        sourceMdn = new TextField("Source MDN", "", 14, TextField.PHONENUMBER);
        phrase = new TextField("Authentication Phrase", "", 10, TextField.PASSWORD);
...
//                f.addCommand(exitCommand);
    ...
    public void commandAction(Command c, Displayable d) {
        if ((c == sendCommand)) {
            if (sourceMdn.getString().equals("") || phrase.getString().equals("") || newPin.getString().equals("")) {
