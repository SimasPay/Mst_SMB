 ...
 * @author Srinu
 */
public class Distribute implements CommandListener {
    ...
    public Distribute(MobileBankingMidlet m, Form pf) {
        parent = m;
        parentForm = pf;
        display = Display.getDisplay(parent);
        f = new Form("Mobile Agent Distribute");
        ...
        if ((c == sendCommand)) {
            if (sourceMdn.getString().equals("") || pin.getString().equals("") || destMdn.getString().equals("") || amount.getString().equals("")) {
                ...
                String url = ConfigurationUtil.getURL() + "SmsServicesServletMA?SMS_sourceMsisdn=" + sourceMdn.getString().trim() + 
                        "&SMS_destMsisdn=" + destMdn.getString().trim() + "&SMS_pin=" + pin.getString().trim() + "&SMS_distributeAmount=" +
                        amount.getString().trim() + "&SMS_serviceName=mobileAgentDistribute";
                try {
                    processRequest(url);
                } catch (Exception e) {
