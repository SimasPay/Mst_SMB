 ...
 * @author Srinu
 */
public class FundsTransfer implements CommandListener {
    ...
    public FundsTransfer(MobileBankingMidlet m, Form pf) {
        parent = m;
        parentForm = pf;
        display = Display.getDisplay(parent);
        f = new Form("Bank Account To Bank Account");
        ...
        message = new TextField("Message", "", 18, TextField.ANY);
...
//      http.setRequestProperty("If-Modified-Since", "Mon, 16 Jul 2001 22:54:26 GMT");
            ...
            if (http.getResponseCode() == HttpConnection.HTTP_OK) {
                ...
                f = new Form("BankAccount to BankAccount");
                si = new StringItem("", "");
                f.append(si);
...
//                f.addCommand(exitCommand);
    ...
    public void commandAction(Command c, Displayable d) {
        if (sourceMdn.getString().equals("") || pin.getString().equals("") || destMdn.getString().equals("") || amount.getString().equals("")
            ...
            if ((c == sendCommand)) {
                String url = ConfigurationUtil.getURL() + "SmsMCashServlet?SMS_sourceMsisdn=" + sourceMdn.getString().trim() +
                        "&SMS_destMsisdn=" + destMdn.getString().trim() +
                        "&SMS_mCashMessage=" + message.getString().trim() + "&SMS_mCashPin=" + pin.getString().trim() +
                        "&SMS_amount=" + amount.getString().trim() + "&SMS_serviceName=mcash_to_mcash";
                try {
                    processRequest(url);
                } catch (Exception e) {
