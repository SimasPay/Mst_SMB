 ...
 * @author ADMIN
 */
public class TopUp implements CommandListener {
    ...
    public TopUp(MobileBankingMidlet m, Form pf) {
        parent = m;
        parentForm = pf;
        display = Display.getDisplay(parent);
        f = new Form("Bank Account Topup");
        ...
        if ((c == sendCommand)) {
            if (sourceMdn.getString().equals("") || pin.getString().equals("")) {
                ...
                String url = ConfigurationUtil.getURL() + "SmsMCashServlet?SMS_sourceMsisdn=" + sourceMdn.getString().trim() +
                        ...
                        "&SMS_bucketType=PR&SMS_amount=" + amount.getString().trim() + "&SMS_serviceName=mcash_topup";
                try {
                    processRequest(url);
                } catch (Exception e) {
