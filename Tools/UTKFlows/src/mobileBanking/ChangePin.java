...
public class ChangePin implements CommandListener {
    ...
    public ChangePin(MobileBankingMidlet m, Form pf) {
        parent = m;
        parentForm = pf;
        display = Display.getDisplay(parent);
        f = new Form("Change Bank Account Pin");
...
//      http.setRequestProperty("If-Modified-Since", "Mon, 16 Jul 2001 22:54:26 GMT");
            ...
            if (http.getResponseCode() == HttpConnection.HTTP_OK) {
                ...
                f = new Form("BankAccount PinChange");
                si = new StringItem("", "");
                f.append(si);
        ...
        if ((c == sendCommand)) {
            if (sourceMdn.getString().equals("") || newPin.getString().equals("") || oldPin.getString().equals("")) {
                ...
                String url = ConfigurationUtil.getURL() + "SmsMCashServlet?SMS_sourceMsisdn=" + sourceMdn.getString().trim() + 
                        "&SMS_oldMCashPin=" + oldPin.getString().trim() + "&SMS_newMCashPin=" + newPin.getString().trim() +
                        "&SMS_serviceName=change_mCash_pin";
                try {
                    processRequest(url);
                } catch (Exception e) {
