 ...
 * @author Srinu
 */
public class CheckBalance implements CommandListener {
    ...
    public CheckBalance(MobileBankingMidlet s, Form pf) {
        parent = s;
        parentForm = pf;
        display = Display.getDisplay(parent);
        f = new Form("Subscriber Check Balance");
...
//      http.setRequestProperty("If-Modified-Since", "Mon, 16 Jul 2001 22:54:26 GMT");
            ...
            if (http.getResponseCode() == HttpConnection.HTTP_OK) {
                ...
                if (str.indexOf("(274)") != -1) {
                    str = str.substring(str.indexOf("(274)"));
                }
                if (str.indexOf("<td>") != -1 && str.indexOf("</td>") != -1) {
                    str = str.substring(str.indexOf("<td>") + "<td>".length(), str.indexOf("</td>"));
                }
                if (str.indexOf("<table>") != -1 && str.indexOf("</table>") != -1) {
                    str = str.substring(str.indexOf("<table>") + "<table>".length(), str.indexOf("</table>"));
                    if (str.indexOf("$(Currency)") != -1) {
                        str = str.substring(0, str.indexOf("$(Currency)")) + str.substring(str.indexOf("$(Currency)") + "$(Currency)".length());
                    }
                }
                f = new Form("Subscriber CheckBalance");
                si = new StringItem("", "");
                f.append(si);
...
//            String url1 = ConfigurationUtil.getURL() + "SmsServicesServlet?SMS_sourceMsisdn="+ sourceMdn.getString()
//                    +"&SMS_pin="+ pin.getString() +"&SMS_serviceName=check_balance";
                String url = ConfigurationUtil.getURL() + "SmsMCashServlet?SMS_sourceMsisdn=" + sourceMdn.getString().trim() +
                        "&SMS_mCashPin=" + pin.getString().trim() + "&SMS_serviceName=mCash_balance_inquiry";
                try {
                    processRequest(url);
                } catch (Exception e) {
