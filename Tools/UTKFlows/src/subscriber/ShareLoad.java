 ...
 * @author Srinu
 */
public class ShareLoad implements CommandListener {
    ...
    public ShareLoad(MobileBankingMidlet s, Form pf) {
        parent = s;
        parentForm = pf;
        display = Display.getDisplay(parent);
        f = new Form("Subscriber Share Load");
...
//      http.setRequestProperty("If-Modified-Since", "Mon, 16 Jul 2001 22:54:26 GMT");
            ...
            if (http.getResponseCode() == HttpConnection.HTTP_OK) {
                ...
                f = new Form("Subscriber ShareLoad");
                si = new StringItem("", "");
                f.append(si);
        ...
        if ((c == sendCommand)) {
            if (sourceMdn.getString().equals("") || pin.getString().equals("") || destMdn.getString().equals("") || amount.getString().equals("")) {
                ...
                String url = ConfigurationUtil.getURL() + "SmsServicesServlet?SMS_sourceMsisdn=" + sourceMdn.getString().trim() +
                        ...
                        "&SMS_rechargeAmount=" + amount.getString().trim() + "&SMS_serviceName=share_load";
                try {
                    processRequest(url);
                } catch (Exception e) {
