 ...
 * @author Srinu
 */
public class Activation implements CommandListener {
    ...
    public Activation(MobileBankingMidlet s, Form pf) {
        parent = s;
        parentForm = pf;
    ...
    public void commandAction(Command c, Displayable d) {
        if ((c == sendCommand)) {
            if (mdn.getString().equals("") || pin.getString().equals("") || phrase.getString().equals("") || number.getString().equals("")) {
                alert = new Alert("Error", "You should enter Mdn, Phrase, Contact number and Pin", null, AlertType.ERROR);
                alert.setTimeout(Alert.FOREVER);
                display.setCurrent(alert);
            } else {
                String url = ConfigurationUtil.getURL() + "SmsServicesServlet?SMS_sourceMsisdn=" + mdn.getString().trim() +
                        "&SMS_pin=" + pin.getString().trim() + "&SMS_contactNumber=" + number.getString().trim() + "&SMS_secretAnswer=" +
                        phrase.getString().trim() + "&SMS_serviceName=subscriber_activation";
                try {
                    processRequest(url);
                } catch (Exception e) {
