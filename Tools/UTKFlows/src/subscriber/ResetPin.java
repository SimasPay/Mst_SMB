 ...
 * @author Srinu
 */
public class ResetPin implements CommandListener {

    private MobileBankingMidlet parent;
    private Display display;
    private Form f, parentForm;
    private TextField sourceMdn, phrase, newPin;
    ...
    public ResetPin(MobileBankingMidlet s, Form pf) {
        parent = s;
        parentForm = pf;
        display = Display.getDisplay(parent);
        f = new Form("Subscriber Reset Pin");

        sourceMdn = new TextField("Source MDN", "", 14, TextField.PHONENUMBER);
        phrase = new TextField("Authentication Phrase", "", 9, TextField.PASSWORD);
        newPin = new TextField("New Pin", "", 6, TextField.PASSWORD);
...
//      http.setRequestProperty("If-Modified-Since", "Mon, 16 Jul 2001 22:54:26 GMT");
            ...
            if (http.getResponseCode() == HttpConnection.HTTP_OK) {
                ...
                f = new Form("Subscriber ResetPin");
                si = new StringItem("", "");
                f.append(si);
...
//                f.addCommand(exitCommand);
    ...
    public void commandAction(Command c, Displayable d) {
        if ((c == sendCommand)) {
            if (sourceMdn.getString().equals("") || newPin.getString().equals("") || phrase.getString().equals("")) {
                alert = new Alert("Error", "You should enter Mdn, Phrase and Pin", null, AlertType.ERROR);
                alert.setTimeout(Alert.FOREVER);
                display.setCurrent(alert);
            } else {
                String url = ConfigurationUtil.getURL() + "SmsServicesServlet?SMS_sourceMsisdn=" + sourceMdn.getString().trim() + 
                        "&SMS_secretAnswer=" + phrase.getString().trim() + "&SMS_newPin=" + newPin.getString().trim() +
                        "&SMS_serviceName=reset_pin";
                try {
                    processRequest(url);
                } catch (Exception e) {
