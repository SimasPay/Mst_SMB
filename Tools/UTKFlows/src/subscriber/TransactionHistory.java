...
public class TransactionHistory implements CommandListener {
    ...
    public TransactionHistory(MobileBankingMidlet s, Form pf) {
        parent = s;
        parentForm = pf;
        display = Display.getDisplay(parent);
        f = new Form("Subscriber Get Transactions");
    ...
    private void processRequest(String url) throws IOException {
        ...
        try {
            ...
            if (http.getResponseCode() == HttpConnection.HTTP_OK) {
                ...
                f = new Form("Subscriber Txns History");
                si = new StringItem("", "");
                f.append(si);
        ...
        if ((c == sendCommand)) {
            if (sourceMdn.getString().equals("") || pin.getString().equals("")) {
                ...
                String url = ConfigurationUtil.getURL() + "SmsServicesServlet?SMS_sourceMsisdn=" + sourceMdn.getString().trim() +
                        "&SMS_pin=" + pin.getString().trim() + "&SMS_serviceName=get_transactions";
                try {
                    processRequest(url);
                } catch (Exception e) {
