 ...
 * @author Srinu
 */
public class ChangePin implements CommandListener {

    private MobileBankingMidlet parent;
    private Display display;
    private Form f, parentForm;
    private TextField sourceMdn, oldPin, newPin;
    ...
    public ChangePin(MobileBankingMidlet s, Form pf) {
        parent = s;
        parentForm = pf;
        display = Display.getDisplay(parent);
        f = new Form("Subscriber Change Pin");

        sourceMdn = new TextField("Source MDN", "", 14, TextField.PHONENUMBER);
        oldPin = new TextField("Old Pin", "", 6, TextField.PASSWORD);
...
//      http.setRequestProperty("If-Modified-Since", "Mon, 16 Jul 2001 22:54:26 GMT");
            ...
            if (http.getResponseCode() == HttpConnection.HTTP_OK) {
                ...
                f = new Form("Subscriber ChangePin");
                si = new StringItem("", "");
                f.append(si);
...
//                f.addCommand(exitCommand);
    ...
    public void commandAction(Command c, Displayable d) {
        if ((c == sendCommand)) {
            if (sourceMdn.getString().equals("") || newPin.getString().equals("") || oldPin.getString().equals("")) {
                alert = new Alert("Error", "You should enter Mdn, Old Pin and New Pin", null, AlertType.ERROR);
                alert.setTimeout(Alert.FOREVER);
                display.setCurrent(alert);
            } else {
                String url = ConfigurationUtil.getURL() + "SmsServicesServlet?SMS_sourceMsisdn=" + sourceMdn.getString().trim() +
                        "&SMS_oldPin=" + oldPin.getString().trim() + "&SMS_newPin=" + newPin.getString().trim() + "&SMS_serviceName=change_pin";
                try {
                    processRequest(url);
                } catch (Exception e) {
