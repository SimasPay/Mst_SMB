...
import com.mfino.application.ExpireCCUpdatesTool;
...
import com.mfino.fix.processor.CCDestinationsProcessor;
import com.mfino.fix.processor.CardInfoProcessor;
import com.mfino.fix.processor.UserProcessor;
...
public class RegistrationUtil {
	...
	public CCRegistrationInfo process(CCRegistrationInfo registrationInfo,
		...
		try {
			...
			log.error("Error processing request " + requestType, e);
	...
	private CCRegistrationInfo getInfo(CCRegistrationInfo ccRegInfo)
		...
		CreditCardDestinationDAO ccdestdao = new CreditCardDestinationDAO();
			...
			if (CmFinoFIX.UserStatus_Active.equals(cardInfo.getCardStatus())
				...
				if (cardInfo.getisConformationRequired()) {
					if (System.currentTimeMillis()- cardInfo.getLastUpdateTime().getTime() > ConfigurationUtil.getCreditcardUpdateExpirationTimeInHrs() * 60 * 60 * 1000) {
						...
						if (expireCCUpdatesTool.revertChanges(cardInfo)) {
	...
	public static CMJSError getSecurityQuestion() {
		...
		try {
			...
			log.error("security question", error);
		} finally {
			HibernateUtil.getCurrentTransaction().rollback();
		}
		return errMsg;
	...
	public static CMJSError getSecurityAnswer() {
		...
		try {
			...
			log.error("security answer", error);
		} finally {
			HibernateUtil.getCurrentTransaction().rollback();
		}
		return errMsg;
	...
	public static boolean checkCompany(Long SubscriberId, String mdn) {
		...
		try {
			...
			log.error("Exception in companyCheck", e);
			HibernateUtil.getCurrentTransaction().rollback();
			return false;
		}
		return true;
