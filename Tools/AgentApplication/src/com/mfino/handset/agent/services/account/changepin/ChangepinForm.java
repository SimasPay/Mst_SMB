 ...
 * @author karthik
 */
public class ChangepinForm extends CustomizedForm implements ActionListener, IResponseReceivedListener {

    UserDataContainer agentDataContainer;
    CustomTextField tfSourcePin;
    CustomTextField tfNewPin;
    CustomTextField tfConfirmPin;
//    Dialog waitingDialog;

    public ChangepinForm(UserDataContainer mcd) {
        this.getStyle().setBgColor(Constants.COLOUR_GRAY);
        this.agentDataContainer = mcd;

        this.previousForm = agentDataContainer.getPreviousFormHolder();
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (ae.getCommand().getId() == 1) {
            ...
            if(!tfNewPin.getText().equals(tfConfirmPin.getText())){
                Dialog d = CustomDialogs.createDialog("new pin and confirm pin are not same", previousForm);
                d.show();
                return;
            }
            ...
            HttpExecutorThread executor = new HttpExecutorThread(agentDataContainer, this);
            executor.start();
            return;
