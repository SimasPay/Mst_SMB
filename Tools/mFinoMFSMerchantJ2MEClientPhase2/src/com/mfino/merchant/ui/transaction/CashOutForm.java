 ...
 * @author sasidhar
 */
public class CashOutForm extends AbstractMfinoConfig implements CommandListener{
...
*/    
    private Form sourceMdnForm = new Form("Enter Phone#");
    private TextField mdn = new TextField("Enter Source Phone#", "", 16, TextField.DECIMAL);
    ...
    private TextField pin = new TextField("Your PIN", "", 6, TextField.PASSWORD);

    private Form subscriberPinForm = new Form("Enter Pin");
    private TextField subscriberPin = new TextField("Subscriber PIN", "", 6, TextField.PASSWORD);
	...
	public CashOutForm(MfinoConfigData mFinoConfigData, Displayable parent){
...
/*		sourceAccountForm.append(sourceAccountChoiceGroup);
		...
		sourceMdnForm.append(mdn);
		sourceMdnForm.addCommand(mFinoConfigData.backCommand);
		sourceMdnForm.addCommand(mFinoConfigData.nextCommand);
		sourceMdnForm.setCommandListener(this);
		...
		subscriberPinForm.append(subscriberPin);
		subscriberPinForm.addCommand(mFinoConfigData.backCommand);
		subscriberPinForm.addCommand(mFinoConfigData.nextCommand);
		subscriberPinForm.setCommandListener(this);
        ...
        display.setCurrent(sourceMdnForm);
	}
	
	public CashOutForm(MfinoConfigData mFinoConfigData)
	{
		super(mFinoConfigData);
	}
	...
	public void commandAction(Command command, Displayable displayable) {
		...
		if (displayable == sourceMdnForm){
			...
			if(command == mFinoConfigData.nextCommand){
				...
				display.setCurrent(subscriberPinForm);
			} else if(command == mFinoConfigData.backCommand){
				display.setCurrent(amountForm);
			}
		}
		else if (displayable == subscriberPinForm){
			if(command == mFinoConfigData.nextCommand){
	            ...
	            if (responseData.getParentTxnId() == null || responseData.getTransferId() == null) {
	            	...
	            	confirmString = "Subscriber with MDN# "+mFinoConfigData.getSourceMdn() + " wants to transfer " + mFinoConfigData.getAmount() + " to your E-Money Account";
				...
				mFinoConfigData.setSourceMdn(sourceMdnHandle);
				display.setCurrent(pinForm);
			}
	...
	private void setValues(){
        ...
        mFinoConfigData.setSourcePin(subscriberPin.getString().trim());
