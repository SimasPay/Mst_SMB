...
import com.mfino.zenith.iso8583.AdditionalAmounts;
import com.mfino.zenith.iso8583.GetConstantCodes;
import com.mfino.zenith.iso8583.processor.IZenithBankISOtoFixProcessor;

public class BalanceInquiryFromZenithProcessor implements IZenithBankISOtoFixProcessor {
	...
	public CFIXMsg process(ISOMsg isoMsg, CFIXMsg request) throws InvalidIsoElementException{
		...
		if(GetConstantCodes.SUCCESS.equals(isoMsg.getString(39)))
		{
			...
			while (amounts.length() > index * entrySize && index < response.getEntries().length) {
				...
				//divide by 100 to get the actual amount that can be showed to user
				entries[index].setAmount(aa.getAmount().divide(new BigDecimal(100)));
				entries[index].setBankAccountType(aa.getAccountType());
				entries[index].setBankAmountType(aa.getAmountType());
	
				if (aa.getCurrencyCode() == CmFinoFIX.ISO8583_CurrencyCode_NAIRA)
