 ...
 * Modified for specific Thales Racal Command Message by Hairi, 2011
 * <h1>How to use</h1>
 * <p>
 ...
 * <dl>
 * <dt>id</dt>
 * <dd>The name of the field. This is used in calls to {}. It should be unique amongst the fields in an FSDMsg.</dd>
 * <dt>length</dt>
 ...
 * <dt>T</dt><dd>Thales Key with optional KeyType</dd>
 * <dt>O</dt><dd>Optionaldatafield, no padding, can be null and is usually predelimiter</dd>

 * Pre delimiter usually menas, existance of ';' right before the field starts.
 ...
 * <dt>ES</dt><dd>End of message separator, ';'.
 *
 * PreDelimiters
 * <dt>SS</dt><dd>Field separator using ';' as the separator. At the start of the field</dd>
 *
 * <dt>SP</dt><dd>Field separator using '%' as the separator. At the start of the field</dd>
 * <dt>SC</dt><dd>Field separator using 'X'1c' as the separator. At the start of the field</dd>
  *
 *
 *
 *
...
public class ThalesMsg  implements Loggeable, Cloneable {
 ...
 /*   public void setSeparator(String separatorName, char separator) {
   ...
   */
        ...
        switch (type.charAt (0)) {
            case 'N':
                if (isSeparated(separator)) {
                    ...
                    value = correctLength(value, length);
                }
                break;
            case 'A':
                if (isSeparated(separator)) {
    ...
    private String correctLength(String value, int len) throws ISOException
    {
    	value = value.trim();
        if (value.length() > len)
        {
        	...
        	return ISOUtil.zeropad(value, len);
    }
    ...
    private char getSeparator(String separator) {
        if (separators.containsKey(separator)) {
            ...
            else return getPreSeparator(separator);
        ...
        throws JDOMException, MalformedURLException, IOException, ISOException
    {
        ...
        // Hack to read different configuration file for bsim reactivation when the command received is JG
        if(this.fields.get("zpkunderlmkreact")!=null){
        	if (keyOff.length() > 0){
        		pack (getSchema (getId (schema), "JG_RA", defaultKey), sb);
        	}
        }
        else{
        	if (keyOff.length() > 0){
        ...
        throws IOException
    {
        ...
        if(is.read(b)>=0)
        {
         try { len = HThalesKeyScheme.getKeyLength((char)b[0]);
