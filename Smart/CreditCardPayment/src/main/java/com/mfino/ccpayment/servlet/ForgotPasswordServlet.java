...
public class ForgotPasswordServlet extends HttpServlet {
	...
	private static Logger log = LoggerFactory.getLogger(ForgotPasswordServlet.class);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(mdn!=null && secAnswer!=null&& "forgotPasswordSecurtiyCheck".equals(requestType)){
			try {
				dispatcher = processRequest(request, response, mdn, secAnswer,
						requestType);		    	
				dispatcher.forward(request, response);		    	
			} catch (HibernateException hiExcep) {
				log.info("Exception",hiExcep);
			}
			finally
			{
				if(session!=null)
	...
	private RequestDispatcher processRequest(HttpServletRequest request,
			HttpServletResponse response, String mdn, String secAnswer,
			String requestType)
