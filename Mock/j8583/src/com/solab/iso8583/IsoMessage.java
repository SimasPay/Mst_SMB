...
public class IsoMessage {
	...
	/** Indicates whether the message is from Mobile8 bank channel gateway. Default is false. */
	public void setIsMobile8(boolean value) {
		IsMobile8 = value;
	}
	
	/** Returns true if the message is from Mobile8 bank channel gateway; default is false. */
	public boolean getIsMobile8() {
		return IsMobile8;
	}
	...
	public String toDebugString(){
		...
		sb.append(String.format("%x", this.getType()));
		for(int i = 2; i <= 150; i ++){
			if(hasField(i)){
				sb.append("\n");
				sb.append(String.format("Field %03d\t", i));
	 ...
	 *            The value to be stored.
	 * @param encoder
	 *            An optional CustomField to encode/decode the value.
	 * @param t
	 *            The ISO type.
	 * @param length
	 *            The length of the field, used for ALPHA and NUMERIC values
	 *            only, ignored with any other type.
	 */
	public void setValue(int index, Object value, CustomField<Object> encoder,
	 ...
	 */
	public void write(OutputStream outs, int lengthBytes, boolean isBinaryLength)
			throws IOException {
		if (lengthBytes > 4) {
		...
		if (lengthBytes > 0) {
			...
			if (isBinaryLength) {
				...
				outs.write(String.format("%0" + lengthBytes + "d", len)
						.getBytes());
			}
		}
		outs.write(data);
	...
	protected byte[] writeInternal() {
		...
		// Adding the last byte 3 for Mobile8 messages
		if(IsMobile8)
		{
			int nLastByte = 0x03;
			char c1 = (char)nLastByte;
