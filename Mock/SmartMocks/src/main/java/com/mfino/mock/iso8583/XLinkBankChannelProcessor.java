...
public class XLinkBankChannelProcessor implements Runnable {
	private byte[] msg;
	private Socket sock;
	private MessageFactory mfact;
	private boolean sentInitialNTMRequest;
	private static Logger	log	= LoggerFactory.getLogger(XLinkBankChannelProcessor.class);
	private IsoMessage isomessage=null;

	public XLinkBankChannelProcessor(byte[] buf, Socket s, MessageFactory mfact,IsoMessage iso) {
		msg = buf;
		sock = s;
		try {
			this.mfact = ConfigParser.createFromClasspathConfig("xlink-config.xml");
		} catch (IOException e) {
	...
	public void fillBasicParams(IsoMessage basicIsoMsg){			
		...
		basicIsoMsg.setValue(7, transmissionDateTime, IsoType.DATE10, 10); //MMDDhhmmss
	...
	public IsoMessage getTopupMsg(String STAN, String RRN, String MDN, BigDecimal amount) {
	...
	public IsoMessage getPostpaidPaymentMsg(String STAN, String RRN, String MDN, BigDecimal amount) {
	...
	public void sendRegisterRequest() throws IOException {			
		...
		registerRequest.setValue(18, "6011", IsoType.NUMERIC, 4);
...
public void sendQueryLowBalRegisteredMDNsRequest() throws IOException {			
