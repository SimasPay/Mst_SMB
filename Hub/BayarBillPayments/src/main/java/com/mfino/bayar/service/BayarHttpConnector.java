...
public class BayarHttpConnector {
	...
	/*public BayarWebServiceResponse sendHttpsRequest(String method, Object parameters) throws MalformedURLException, SocketTimeoutException {
		...
		//HttpPost httpPost = new HttpPost(url + "/" + method);
		TrustMgr tstmgr = new TrustMgr();
		HttpsURLConnection connection = (HttpsURLConnection) (new URL(url + "/" + method)).openConnection();
		connection.setSSLSocketFactory(tstmgr.getSSLFactory());
		connection.setHostnameVerifier(tstmgr.getHostNameVerifier());
