 ...
 *
 */
public class BillPayInquiryProcessor extends BillPaymentsBaseServiceImpl implements BSMProcessor {
	...
	public MCEMessage constructRequestMessage(MCEMessage mceMessage){
		...
		toOperator.setOperatorCode(CmFinoFIX.OperatorCodeForRouting_CBOSS);//routing code 
		
		replyMessage.setRequest(mceMessage.getRequest());
		replyMessage.setResponse(toOperator);
		replyMessage.setDestinationQueue("jms:bsmBillPayInquiryResponseQueue?disableReplyTo=true");
