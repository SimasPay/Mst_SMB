...
public class MissingResponseHandlerServiceDefaultImpl {
	...
	public MCEMessage processMessage(MCEMessage mesg) {
		...
		// also write for reversal also not replied by bank
			...
			reversalFixMsg.setBankSystemTraceAuditNumber((moneyTransferToBank.getTransactionID() % 1000000) + "");
			log.info("message to be reversed is "+reversalFixMsg.getBankSystemTraceAuditNumber());
			
			// removing the +1 that is being done here since it is already done in
			// ConversionToReversalRequestProcessor
		
			reversalFixMsg.setTransactionID(moneyTransferToBank.getTransactionID());
			reversalFixMsg.setBankRetrievalReferenceNumber(moneyTransferToBank.getTransactionID() + "");
			mesg.setResponse(reversalFixMsg);
		}
