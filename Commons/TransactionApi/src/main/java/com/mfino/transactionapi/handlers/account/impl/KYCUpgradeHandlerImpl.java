...
public class KYCUpgradeHandlerImpl extends FIXMessageHandler implements KYCUpgradeHandler{
	...
	private void sendSms(XMLResult result, Subscriber subscriber) {
		...
		query.setLanguage(new Integer(String.valueOf(subscriber.getLanguage())));
		...
		if(CollectionUtils.isNotEmpty(notifications) && (! (notification.getIsactive() != null && notification.getIsactive() != 0)) ){
			...
			notificationWrapper.setLanguage(new Integer(String.valueOf(subscriber.getLanguage())));
