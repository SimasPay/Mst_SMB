...
public class ScheduleTemplateProcessorImpl extends BaseFixProcessor implements
		ScheduleTemplateProcessor {
	...
	private void updateEntity(ScheduleTemplate scheduleTemplate,
			CmFinoFIX.CMJSScheduleTemplate.CGEntries e) throws ParseException {
		...
		scheduleTemplate.setMspid(new BigDecimal(1));
		CronExpressionCreator tCron = new CronExpressionCreator();
		if (e.getModeType() != null) {
			tCron.setRecurring(e.getModeType().equals(
					CmFinoFIX.ModeType_Minutes)
					|| e.getModeType().equals(CmFinoFIX.ModeType_Hourly)
					|| e.getModeType().equals(CmFinoFIX.ModeType_Daily));
	...
	private void updateMessage(ScheduleTemplate scheduleTemplate,
			CMJSScheduleTemplate.CGEntries entry) {
		 ...
		 * //entry.setTimeTypeText
		 * (EnumTextService.getEnumTextValue(CmFinoFIX.TagID_TimeType, null,
		 * scheduleTemplate.getTimeType())); }
		 */
	...
	public CFIXMsg process(CFIXMsg msg) throws Exception {
		...
		ScheduleTemplateDAO dao = DAOFactory.getInstance()
				.getScheduleTemplateDao();
		if (CmFinoFIX.JSaction_Select.equalsIgnoreCase(realMsg.getaction())) {
			...
			for (CMJSScheduleTemplate.CGEntries e : entries) {
				...
				log.info("Schedule Template: " + st.getId()
						+ " edit completed by user:"
