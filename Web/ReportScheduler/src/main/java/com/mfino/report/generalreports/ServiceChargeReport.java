...
public class ServiceChargeReport extends OfflineReportBase {
	private Logger log = LoggerFactory.getLogger(this.getClass());
	private static final int NUM_COLUMNS = 4;
	private String HEADER_ROW = "# ,Date ,Service name ";
	private List<ServiceChargeTransactionLog> sctlogs = null;
	private Map<Long, List<ServiceChargeTransactionLog>> serviceSCTLMap = null;
	...
	protected File run(Date start, Date end, ReportBaseData basedata) {
		...
		try {
			...
			if (!sctlogs.isEmpty()) {
				serviceSCTLMap = getServiceSCTLMap(sctlogs);
				int seq = 1;
				for (Service service : data.services) {
					List<ServiceChargeTransactionLog> sctl = serviceSCTLMap.get(service.getID());
