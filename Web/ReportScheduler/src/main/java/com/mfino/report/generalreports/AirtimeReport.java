...
public class AirtimeReport extends OfflineReportBase {
	private Logger log = LoggerFactory.getLogger(this.getClass());
	private String AIRTIME_HEADER_ROW = "# , MDN, TransactionType, Amount";
	private int AIRTIME_NUM_COLUMNS = AIRTIME_HEADER_ROW.split(",").length;
	...
	protected File run(Date start, Date end, Map<String,Integer[]> countMap) {
		...
		String agentstr = getFormatString(AIRTIME_NUM_COLUMNS);
		if(end == null){
		...
		log.info("Processing Airtime report StartTime:"+startTime+"  and EndTime:"+endTime);
		try {
			if(StringUtils.isBlank(reportName)){
				OfflineReportDAO offlineReportDAO =DAOFactory.getInstance().getOfflineReportDAO();
				reportName =  offlineReportDAO.getByReportClass("com.mfino.report.generalreports.AirtimeReport").getName();
			}
			...
			airtimeXlsReport.addHeaderRow(AIRTIME_HEADER_ROW);
			...
			airtimeXlsReport.writeToFileStream(reportFile, AIRTIME_HEADER_ROW, reportName);
		}catch (Exception e) {
