...
mFino.page.groups = function(config){

    var detailsForm = new mFino.widget.GroupDetails(Ext.apply({
        height : 173
    }, config));

    var searchBox = new mFino.widget.GroupSearchForm(Ext.apply({
        height : 170
    }, config));
    ...
    var gridViewForm = new mFino.widget.FormWindow(Ext.apply({
        form : new mFino.widget.GroupViewForm(config),
        height:150,
        width: 400,
        mode:'close'
    },config));

    var grid = new mFino.widget.GroupGrid(Ext.apply({
        layout:'fit',
        title : _('Group Search Results'),
        frame:true,
        loadMask:true,
        height: 485,
        width: 925
    ...
    var panelCenter = new Ext.Panel({
        ...
        items : [
        {
            ...
            anchor : "100%",
            tbar : [
            '<b class= x-form-tbar>' + _('Group Details') + '</b>',
            '->',
            {
    ...
    grid.action.on({
        action:function(grid, record, action, row, col) {
            ...
            if(action === 'mfino-button-View'){
                gridViewForm.setTitle( _("View Group"));
