...
mFino.page.lop = function(config){

    var detailsForm = new mFino.widget.LOPDetails(Ext.apply({
        }, config));

    var searchBox = new mFino.widget.LOPSearchForm(Ext.apply({
        height : 200
    }, config));
    ...
    var grid = new mFino.widget.LOPGrid(Ext.apply({
    ...
    var panelCenter = new Ext.Panel({
        ...
        items : [
        {
            ...
            anchor : "100%",
            tbar : [
            '<b class= x-form-tbar>' + _('LOP Details') + '</b>',
            '->',
            {
