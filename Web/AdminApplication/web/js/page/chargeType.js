...
mFino.page.chargeType = function(config){

    var detailsForm = new mFino.widget.ChargeTypeDetails(Ext.apply({
        height : 173
    }, config));

    var searchBox = new mFino.widget.ChargeTypeSearchForm(Ext.apply({
        height : 170
    }, config));

    var gridAddForm = new mFino.widget.FormWindow(Ext.apply({
        form : new mFino.widget.ChargeTypeForm(config),
        height : 150,
        width: 400
    },config));

    var gridViewForm = new mFino.widget.FormWindow(Ext.apply({
        form : new mFino.widget.ChargeTypeViewForm(config),
        height:150,
        width: 400,
        mode:'close'
    },config));

    var grid = new mFino.widget.ChargeTypeGrid(Ext.apply({
        layout:'fit',
        title : _('Charge Type Search Results'),
        frame:true,
        loadMask:true,
        height: 485,
        width: 925
    ...
    var panelCenter = new Ext.Panel({
        ...
        items : [
        {
            ...
            anchor : "100%",
            tbar : [
            '<b class= x-form-tbar>' + _('Charge Type Details') + '</b>',
            '->',
            {
                iconCls: 'mfino-button-pocket-add',
                tooltip : _('Add Charge Type'),
                text : _('New'),
                itemId: 'chargetype.details.add',
                handler: function(){
                    var record = new grid.store.recordType();
                    gridAddForm.setTitle(_("Add Charge Type"));
    ...
    grid.action.on({
        action:function(grid, record, action, row, col) {
            if(action === 'mfino-button-edit'){
                gridAddForm.setTitle( _("Edit Charge Type"));
            ...
            if(action === 'mfino-button-View'){
                gridViewForm.setTitle( _("View Charge Type"));
