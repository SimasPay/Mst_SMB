...
mFino.widget.SubscriberLakupandaiFormView = function (config) {
    ...
    mFino.widget.SubscriberLakupandaiFormView.superclass.constructor.call(this, localConfig);
};


Ext.extend(mFino.widget.SubscriberLakupandaiFormView, Ext.form.FormPanel, {
            ...
            items : [
            {
                ...
                items : [
                {
                    ...
                    items: [
							...
							//        	this.findParentByType('SubscriberLakupandaiFormView').onMDN(field);
							        }
							    },
							...
							
							{
		                      	 ...
		                      	 listeners: {
		                            change: function(field) {
//		                           	 this.findParentByType('SubscriberLakupandaiFormView').onDOBSelect(field);
		                           	                          }}
		                   	},
		                   	{
                               xtype : 'textfield',
...
//                               id : 'sub.form.KYCLevel',
                               ...
                               listeners: {
                                   select: function(field) {
                                   	...
                                   	var subscriberForm = this.findParentByType('SubscriberLakupandaiFormView');
...
//		                        value : CmFinoFIX.MDNStatus.Initialized,
		                        listeners : {
		                            select :  function(field){
		                                ...
		                                this.findParentByType('SubscriberLakupandaiFormView').onStatusDropdown(status);
		                            }
		                        }
                    }
...
//            }
        ...
        mFino.widget.SubscriberLakupandaiFormView.superclass.initComponent.call(this);
...
//            }
    ...
    onIDLifeTime:function(value){
    	 if(value){
    		 this.find('itemId','sub.form.IdValidUntil')[0].reset();
    		 this.find('itemId','sub.form.IdValidUntil')[0].setDisabled(true);
    		 this.find('itemId','sub.form.IdValidUntil')[0].allowBlank=true;
    	 }else{
    		 this.find('itemId','sub.form.IdValidUntil')[0].setDisabled(false)
    		 this.find('itemId','sub.form.IdValidUntil')[0].allowBlank=false;
    	 }
    },
...
//        var value=field.getValue();
    ...
    setRecord : function(record){
		...
		if(record.get(CmFinoFIX.message.JSSubscriberMDN.Entries.KTPDocumentPath._name)!=null){
			...
			docFullPath=docFullPath.replace("\\","/");
			if(mFino.widget.SubscriberLakupandaiFormView.path == null || mFino.widget.SubscriberLakupandaiFormView.path == '' || mFino.widget.SubscriberLakupandaiFormView.path == undefined){
					mFino.widget.SubscriberLakupandaiFormView.path=docFullPath.substring(0,docFullPath.lastIndexOf('/')+1);
			}
...
//        this.find('itemId','sub.form.creditcheck')[0].disable();
	...
	onDomisiliChange: function(isSameAddress){
			...
			if(!mFino.widget.SubscriberLakupandaiFormView.isLoadRecord){
				
				if(!isSameAddress){
				this.form.items.get("sub.form.plotno").setValue('');
				this.form.items.get("sub.form.RT").setValue('');
				this.form.items.get("sub.form.RW").setValue('');
				this.form.items.get("sub.form.ProvincialCom").setValue('');
				this.form.items.get("sub.form.CityCom").setValue('');
				this.form.items.get("sub.form.DistrictCom").setValue('');
				this.form.items.get("sub.form.VillageCom").setValue('');
				this.form.items.get("sub.form.zipcode").setValue('');
				
				this.form.items.get("sub.form.plotno").setDisabled(false);
				this.form.items.get("sub.form.RT").setDisabled(false);
				this.form.items.get("sub.form.RW").setDisabled(false);
				this.form.items.get("sub.form.ProvincialCom").setDisabled(false);
				this.form.items.get("sub.form.CityCom").setDisabled(false);
				this.form.items.get("sub.form.DistrictCom").setDisabled(false);
				this.form.items.get("sub.form.VillageCom").setDisabled(false);
				this.form.items.get("sub.form.zipcode").setDisabled(false);
			}else{
				this.form.items.get("sub.form.plotno").setDisabled(true);
				this.form.items.get("sub.form.RT").setDisabled(true);
				this.form.items.get("sub.form.RW").setDisabled(true);
				this.form.items.get("sub.form.ProvincialCom").setDisabled(true);
				this.form.items.get("sub.form.CityCom").setDisabled(true);
				this.form.items.get("sub.form.DistrictCom").setDisabled(true);
				this.form.items.get("sub.form.VillageCom").setDisabled(true);
				this.form.items.get("sub.form.zipcode").setDisabled(true);
			}
			
		}else{
			mFino.widget.SubscriberLakupandaiFormView.isLoadRecord=false;
		}
...
//         checked: true
     ...
     listeners: {
         change: function(radiogroup, radio){
             ...
             this.findParentByType("SubscriberLakupandaiFormView").onDomisiliChange(isSameAddress);
//             alert(visible)
...
//    width: 600,
    ...
    },
    {
		...
		listeners: {
            check: function() {
                //this.findParentByType("SubscriberLakupandaiFormView").onCheckEmail();  //commented to fix #3381
            }
        }
    }]
    }]
...
//    width: 600,
    ...
    items : [{
        ...
        items: [
        {
            ...
            listeners: {
                check: function() {
                    this.findParentByType("SubscriberLakupandaiFormView").onSecurityLockClick();
                }
            }
        },
        {
            ...
            listeners: {
                check: function() {
                    this.findParentByType("SubscriberLakupandaiFormView").onAbsoluteLockClick();
                }
            }
        },
//        {
...
//        },
        {
            ...
            listeners: {
                check: function() {
                    this.findParentByType("SubscriberLakupandaiFormView").onSuspendClick();
                }
            }
        },
        {
            ...
            listeners: {
                check: function() {
                    this.findParentByType("SubscriberLakupandaiFormView").onNoFundMovementClick();
                }
            }
        }]
    }]
	...
	//    id : 'sub.form.KYCLevel',
        ...
        listeners: {
            select: function(field,record) {
            	this.findParentByType('SubscriberLakupandaiFormView').onWork(field,record);
//            	
            }
        }
...
//		        itemId : 'SMS',
				...
				listeners: {
			         check: function(thisObj,checked){
			        	 this.findParentByType('SubscriberLakupandaiFormView').onIDLifeTime(checked);
//			             alert(checked)
         ...
         listeners:{
         	 afterrender: function(component) {
         	      ...
         	      component.getEl().on('click', function() { 
					    ...
					    mFino.widget.SubscriberLakupandaiFormView.prototype.showImage(component.getValue())
...
//    	height:5 
     },
     {
			 ...
			 listeners: {
				 	select: function(field,record) {
				 	this.findParentByType('SubscriberLakupandaiFormView').onProvince2(field,record);
			    }
			}
      },
      {
       	...
       	editable:false,
            ...
            hiddenName :CmFinoFIX.message.JSSubscriberMDN.Entries.City._name,
			 listeners: {
				 	select: function(field,record) {
				 	this.findParentByType('SubscriberLakupandaiFormView').onProvinceRegion2(field,record);
			    }
			}
       },
       {
			 ...
			 listeners: {
				 	select: function(field,record) {
				 	this.findParentByType('SubscriberLakupandaiFormView').onDistrict2(field,record);
			    }
			}
        },
        {
...
Ext.reg("SubscriberLakupandaiFormView", mFino.widget.SubscriberLakupandaiFormView);
