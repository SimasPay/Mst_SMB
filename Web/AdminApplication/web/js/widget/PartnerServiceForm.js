...
mFino.widget.PartnerServiceForm = function (config) {
    var localConfig = Ext.apply({}, config);
    localConfig = Ext.applyIf(localConfig, {
        bodyStyle:'padding:5px 5px 0',
        frame : true
    });
    mFino.widget.PartnerServiceForm.superclass.constructor.call(this, localConfig);
};

Ext.extend(mFino.widget.PartnerServiceForm, Ext.FormPanel, {
        ...
        this.items = [
          {
              ...
              listeners: {
	              select: function(field) {
        	  		  this.findParentByType('partnerserviceform').getServices(field.getValue());
	                  this.findParentByType('partnerserviceform').getPartners();
	              }
	          }              
          },  
          {
              ...
              listeners: {
	              select: function(field) {
	                  this.findParentByType('partnerserviceform').getPartners();
	                  this.findParentByType('partnerserviceform').getDcts(field.value);
	              }
	          }              
          },  
          {
...
//              allowBlank: false,
          ...
          }, 
          {
              ...
              listeners: {
                  blur: function(field) {
                      this.findParentByType('partnerserviceform').checkParent(field);
                  }
              }
          },          
	      {
	          ...
	          allowBlank: (!EMONEY_ENABLED),
			  emptyText : _('<select one..>'),
	          itemId : 'ps.form.collectorpocket',
	          triggerAction: "all",
	          forceSelection : (EMONEY_ENABLED),
	          lastQuery: '',
              ...
              listeners: {
                  focus: function(field) {
                      this.findParentByType('partnerserviceform').checkSize(field);
                  }
              }
	      },
	      {
	      ...
	      },	
          {
              ...
              listeners : {
                  select :  function(status){
                      ...
                      this.findParentByType('partnerserviceform').onStatusDropdown(s);
                  }
              }
          },
          {
        ...
        mFino.widget.PartnerServiceForm.superclass.initComponent.call(this);
...
//            	this.settlementConfigGrid.setScheduleStatus(CmFinoFIX.SchedulerStatus.Rescheduled);
    ...
    setRecord : function(record){
    	...
    	/*sp_combo.store.baseParams[CmFinoFIX.message.JSPocket.SubscriberIDSearch._name] = 
    				this.record.get(CmFinoFIX.message.JSPartnerServices.SubscriberID._name);
...
//    		this.find('itemId','ps.form.status')[0].enable();
		...
		dct_combo.setRawValue(this.record.get(CmFinoFIX.message.JSPartnerServices.Entries.DistributionChainName._name));
    },
...
Ext.reg("partnerserviceform", mFino.widget.PartnerServiceForm);
