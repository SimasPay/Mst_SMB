...
mFino.widget.BulkLOPAddForm = function (config) {
    ...
    mFino.widget.BulkLOPAddForm.superclass.constructor.call(this, localConfig);
};

Ext.extend(mFino.widget.BulkLOPAddForm, Ext.FormPanel, {
    initComponent : function () {
        ...
        },
        {
            ...
            listeners: {
                blur: function(){
                    this.findParentByType("bulklopaddform").evaluateActualPaidValueAndSet();
                }
            }
        },
        {
            ...
            listeners: {
                blur: function(){
                    this.findParentByType("bulklopaddform").evaluateActualValueValueAndSet();
                }
            }
        },
        {
        ...
        mFino.widget.BulkLOPAddForm.superclass.initComponent.call(this);
    ...
    save : function(formWindow,actualPaid){
        //if(this.getForm().isValid())
            var msg= new CmFinoFIX.message.JSGenerateBulkLOP();
            ...
            if(this.merchantMdn){
            //    msg.SetSourceMDN(this.merchantMdn);
            }
            if(lopComment){
            	msg.m_pComment = lopComment;
            }
            msg.m_pMerchantID = this.merchantId;
            bulklopgrid = this.items.get("bulklop");
            records = bulklopgrid.getStore().getModifiedRecords();
            if(records.length<1)
            {
                return;// to be replaced by message
            }
            var str = "";
            for(i =0; i<records.length;i++)
            {
                str += records[i].data.ID +",";
                str += records[i].data.Denomination +",";
                str += records[i].data.Units+"|";
            }
    ...
    setLimits : function(mdn,MaxWeeklyPurchaseAmount,CurrentWeeklyPurchaseAmount,AvailableforLOP,LOPCommission,GroupId,maxAmountPerTransaction, merchantID){
        ...
        bulklopgrid = this.items.get("bulklop");
        bulklopgrid.store.baseParams[CmFinoFIX.message.JSPocket.MerchantIDSearch._name] = merchantID;
        bulklopgrid.store.baseParams[CmFinoFIX.message.JSPocket.Commodity._name] = CmFinoFIX.Commodity.Airtime;
        bulklopgrid.store.baseParams[CmFinoFIX.message.JSPocket.PocketType._name] = CmFinoFIX.PocketType.SVA;
        this.merchantId = merchantID;
        bulklopgrid.store.lastOptions = {
            params : {
                start : 0,
                limit : CmFinoFIX.PageSize.Default
            }
        };
        bulklopgrid.store.load(bulklopgrid.store.lastOptions);
    },
...
Ext.reg("bulklopaddform", mFino.widget.BulkLOPAddForm);
