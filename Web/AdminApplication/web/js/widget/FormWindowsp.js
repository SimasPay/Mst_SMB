...
mFino.widget.FormWindowsp = function (config){
    ...
    mFino.widget.FormWindowsp.superclass.constructor.call(this, localConfig);
};

Ext.extend(mFino.widget.FormWindowsp, Ext.Window, {
    initComponent : function(){
        ...
        /*{
        ...
        } ,*/
         {
            itemId: "further",
            text: _('LANJUT'),
            handler: this.onFurther.createDelegate(this)
        },
        {
            itemId : "save",
            text: _('SIMPAN'),
            handler: this.onSIMPAN.createDelegate(this)
        },
        {
            itemId : "edit",
            text: _('SIMPAN'),
        ...
        },
        {
            itemId: "cancel2",
            text: _('BATAL'),
            handler: this.onCancel2.createDelegate(this)
        },
        {
            itemId: "close2",
            text: _('DEKAT'),
        ...
        mFino.widget.FormWindowsp.superclass.initComponent.call(this);
    ...
    setMode : function(mode){
        Ext.each(this.buttons, function(item){
            item.hide();
            if((mode === "edit" || mode === "add") && (item.itemId === "ok" || item.itemId === "cancel")){
                item.show();
            }
            ...
            if(mode === "addagent" && (item.itemId === "further" || item.itemId === "cancel2")){
                item.show();
            }
            if(mode === "addagentdata" && (item.itemId === "save" || item.itemId === "cancel2")){
                item.show();
            }
            if(mode === "editagentdata" && (item.itemId === "edit" || item.itemId === "cancel2")){
                item.show();
            }
            if(mode === "closeagent" && (item.itemId === "close2")){
                item.show();
            }
        });
    },
    ...
    onFurther:function(){
        if(this.form.getForm().isValid()){
        	this.form.further(this);
           }
    },
    
    onSIMPAN:function(){
        if(this.form.getForm().isValid()){
        	this.form.simpan(this);
           }
    },
    onEDIT : function(){
    	this.getEl().mask();
