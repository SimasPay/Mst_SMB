 ...
 */
mFino.widget.ProductReferralGrid = function (config) {
    
    var localConfig = Ext.apply({}, config);

    var store = new FIX.FIXStore(config.dataUrl, CmFinoFIX.message.JSProductReferral);

    if(mFino.auth.isEnabledItem('Product.grid.edit')){
        ...
        this.action = new Ext.ux.grid.RowActions({
            ...
            {
                iconCls:'mfino-button-View',
                tooltip: _('View Product Referral'),
                align:'right'
            }
            ]
        });
    ...
    mFino.widget.ProductReferralGrid.superclass.constructor.call(this, localConfig);
};

Ext.extend(mFino.widget.ProductReferralGrid, Ext.grid.GridPanel, {
    initComponent : function () {
        if(this.store){
            this.store.on("load", this.onStoreChange.createDelegate(this));
            this.store.on("update", this.onStoreChange.createDelegate(this));
        }
        mFino.widget.ProductReferralGrid.superclass.initComponent.call(this);
    },
