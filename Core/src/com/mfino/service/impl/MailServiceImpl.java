...
public class MailServiceImpl  implements MailService{
    ...
    private static final String MAIL_SMTP_SOCKET_FACTORY_CLASS =
   ...
   * 
   */
  // FIXME: Catch this exception and throws our own exception. Need to keep the
  ...
  public void sendMailwithAttachment(String toAddress, String toName, String subject,String message, String filePath) throws EmailException 
  {
...
//    FixMessageSerializer fms = new FixMessageSerializer(ConfigurationUtil.getBackendURL());
//
...
//    return fms.send(subNotification);
  ...
  { 
	  asyncSendEmailwithAttachment(toAddress, toName, subject, message, filePath, null);
  ...
  public void asyncSendEmailwithAttachment(final String toAddress, final String toName, final String subject,
  	...
  	threadPool.execute(new Runnable() {			
			...
			public void run(){
				...
				{
					notificationLogDetails = notificationLogDetailsService.getNotificationLogDetailsById(notificationLogDetailsID);
				}
				try {
					sendMailwithAttachment(toAddress, toName, subject, message, filePath);
					if(notificationLogDetails != null)
			    	{
			    		notificationLogDetails.setStatus(CmFinoFIX.SendNotificationStatus_Success);
			    		notificationLogDetailsService.saveNotificationLogDetails(notificationLogDetails);
			    	}
  ...
  public void generateEmailVerificationMail(Subscriber subscriber, String email) {
		...
		mailBody = mailBody.replace("$(subscriberID)", subscriber.getID().toString());
		mailBody = mailBody.replace("$(email)", email);
		asyncSendEmail(email, subscriber.getFirstName() + " " + subscriber.getLastName(), ConfigurationUtil.getEmailVerificationSubject(), mailBody);		
	}
}
