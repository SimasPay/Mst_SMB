...
var WorkspaceToolbar = Backbone.View.extend({
    ...
    initialize: function(args) {
        ...
        _.bindAll(this, "call", "reflect_properties", "run_query",
            ...
            "clicked_cell_drillthrough","clicked_cell_drillthrough_pdf_export","activate_buttons", "switch_to_mdx","post_mdx_transform");
    ...
    check_modes: function(source) {
        ...
        if (!$(source).hasClass('on')) {
            ...
            if ($(source).hasClass('drillthrough_export')) {
                ...
                $(this.workspace.el).find(".query_scenario, .drillthrough, .drillthrough_pdf_export").removeClass('on');

            }else if ($(source).hasClass('drillthrough_pdf_export')) {
                $(this.workspace.el).find("td.data").addClass('cellhighlight').unbind('click').click(this.clicked_cell_drillthrough_pdf_export);
                $(this.workspace.el).find(".query_scenario, .drillthrough_export, .drillthrough").removeClass('on');             
                ...
                $(this.workspace.el).find(".query_scenario, .drillthrough_export, .drillthrough_pdf_export").removeClass('on');

            } else if ($(source).hasClass('query_scenario')) {
                this.workspace.query.scenario.activate();
                $(this.workspace.el).find(".drillthrough, .drillthrough_export, .drillthrough_pdf_export").removeClass('on');
            }
        }
    ...
    export_drillthrough_pdf: function(event) {
        $(event.target).toggleClass('on');
        this.check_modes($(event.target));        
    },
    
     clicked_cell_drillthrough_pdf_export: function(event) {
        $target = $(event.target).hasClass('data') ?
            $(event.target).find('div') : $(event.target);
        var pos = $target.attr('rel');     
        (new DrillthroughPdfModal({
            workspace: this.workspace,
            maxrows: 10000,
            title: "Drill-Through to Pdf",
            action: "pdfexport",
            position: pos,
    ...
    export_xls: function(event) {
        if (this.workspace.query) {
            (new ExportExcel({ query: this.workspace.query })).render().open();
        }
    },
    
    export_pdf: function(event) {
        if (this.workspace.query) {
            (new ExportPdf({ query: this.workspace.query })).render().open();
        }
    },
  ...
  /*  export_pdf: function(event) {
        window.location = Settings.REST_URL +
            Saiku.session.username + "/query/" + 
            this.workspace.query.id + "/export/pdf";
    }, */
