...
public class FlattenedCellSetFormatter implements ICellSetFormatter {
	 ...
	 */
	private static Iterable<Cell> cellIter(final int[] pageCoords,
			final CellSet cellSet) {
		return new Iterable<Cell>() {
			public Iterator<Cell> iterator() {
				final int[] axisDimensions = new int[cellSet.getAxes().size()
						- pageCoords.length];
				...
				final CoordinateIterator coordIter = new CoordinateIterator(
						axisDimensions, true);
					...
					public Cell next() {
						...
						final AbstractList<Integer> intList = new AbstractList<Integer>() {
							...
							public Integer get(final int index) {
								return index < ints.length ? ints[index]
	...
	public Matrix format(final CellSet cellSet) {
		...
		if (cellSet.getAxes().size() > 2) {
			...
			for (final int[] pageCoords : CoordinateIterator
					.iterate(dimensions)) {
				matrix = formatPage(cellSet, pageCoords, columnsAxis,
						columnsAxisInfo, rowsAxis, rowsAxisInfo);
			}
		} else {
			matrix = formatPage(cellSet, new int[] {}, columnsAxis,
					columnsAxisInfo, rowsAxis, rowsAxisInfo);
		}
	...
	private AxisInfo computeAxisInfo(final CellSetAxis axis) {
		...
		final AxisInfo axisInfo = new AxisInfo(axis.getAxisMetaData()
				.getHierarchies().size());
		int p = -1;
		for (final Position position : axis.getPositions()) {
			...
			for (final Member member : position.getMembers()) {
				++k;
				final AxisOrdinalInfo axisOrdinalInfo = axisInfo.ordinalInfos
						.get(k);
				if (!axisOrdinalInfo.getDepths().contains(member.getDepth())) {
					axisOrdinalInfo.getDepths().add(member.getDepth());
					axisOrdinalInfo.addLevel(member.getDepth(),
							member.getLevel());
	 ...
	 */
	private Matrix formatPage(final CellSet cellSet, final int[] pageCoords,
			final CellSetAxis columnsAxis, final AxisInfo columnsAxisInfo,
			final CellSetAxis rowsAxis, final AxisInfo rowsAxisInfo) {
			...
			for (int x = 0; x < xOffsset; x++) {
				...
				for (int y = 0; y < yOffset; y++) {
					...
					if (y == yOffset - 1) {
						...
						memberInfo.setProperty("__headertype",
								"row_header_header");
						memberInfo.setProperty("levelindex",
								"" + levels.indexOf(xLevel));
						memberInfo.setProperty("hierarchy", ""
								+ xLevel.getHierarchy().getUniqueName());
						memberInfo.setProperty("uniqueName",
								"" + xLevel.getUniqueName());
					}
		...
		for (int yy = matrix.getMatrixHeight(); yy > matrix.getOffset(); yy--) {
			for (int xx = 0; xx < headerwidth - 1; xx++) {
				if (matrix.get(xx, yy - 1) != null
						&& matrix.get(xx, yy) != null
						&& matrix.get(xx, yy - 1).getRawValue()
								.equals(matrix.get(xx, yy).getRawValue())) {
		...
		for (final Cell cell : cellIter(pageCoords, cellSet)) {
			...
			//
			// if (headerCell instanceof MemberCell && ((MemberCell)
			// headerCell).getUniqueName() != null) {
			// } else {
			...
			//
			// cellInfo.setProperty(proplist.get(i).getName(),
			// cell.getPropertyValue(proplist.get(i)).toString());
			// }
			//
	 ...
	 */
	private void populateAxis(final Matrix matrix, final CellSetAxis axis,
			final AxisInfo axisInfo, final boolean isColumns,
			final int oldoffset) {
		...
		for (int i = 0; i < axis.getPositions().size(); i++) {
			...
			for (int j = 0; j < memberList.size(); j++) {
				...
				final AxisOrdinalInfo ordinalInfo = axisInfo.ordinalInfos
						.get(j);
				...
				final int y = yOffset
						+ ordinalInfo.depths.indexOf(member.getDepth());
				members[y] = member;
			...
			for (int y = 0; y < members.length; y++) {
				...
				if (index >= 0) {
					final AxisOrdinalInfo ordinalInfo = axisInfo.ordinalInfos
							.get(index);
					int depth_i = ordinalInfo.getDepths().indexOf(
							member.getDepth());
					if (depth_i > 0) {
						expanded = true;
					}
				...
				if (member != null) {
					...
					memberInfo.setFormattedValue(member.getCaption()); // First
																		// try
																		// to
																		// get a
																		// formatted
																		// value
					memberInfo.setParentDimension(member.getDimension()
							.getName());
					...
					if (y > 0) {
						for (int previ = y - 1; previ >= 0; previ--) {
							if (prevMembers[previ] != null) {
								...
								memberInfo
										.setRightOfDimension(prevMembers[previ]
					...
					if (member.getParentMember() != null) {
						memberInfo.setParentMember(member.getParentMember()
								.getUniqueName());
					}
				...
				if (index >= 0) {
					...
					while (depth_i > 0) {
						depth_i--;
						Level l = ordinalInfo.getLevel(ordinalInfo.getDepths()
								.get(depth_i));
						Member parent = member.getParentMember();
						while (l != null
								&& parent != null
								&& !parent.getLevel().getUniqueName()
						...
						if (parent != null) {
							...
							pInfo.setFormattedValue(parent.getCaption()); // First
																			// try
																			// to
																			// get
																			// a
																			// formatted
																			// value
							pInfo.setParentDimension(parent.getDimension()
									.getName());
							pInfo.setUniquename(parent.getUniqueName());
						} else {
							pInfo.setRawValue("");
							pInfo.setFormattedValue(""); // First try to get a
															// formatted value
							pInfo.setParentDimension(member.getDimension()
									.getName());
							pInfo.setUniquename("");
						}
