...
import org.saiku.service.util.export.DrillThroughPdfExport;
import org.saiku.service.util.export.ExcelExporter;
import org.saiku.service.util.export.PdfExporter;
...
public class OlapQueryService implements Serializable {
	...
	private static final Logger log = LoggerFactory
			.getLogger(OlapQueryService.class);
	...
	public SaikuQuery createNewOlapQuery(String queryName, SaikuCube cube) {
		try {
			...
			OlapConnection con = olapDiscoverService.getNativeConnection(cube
					.getConnectionName());

			if (cub != null) {
				IQuery query = new OlapQuery(new Query(queryName, cub), con,
						cube);
	...
	public SaikuQuery createNewOlapQuery(String name, String xml) {
		try {
			...
			OlapConnection con = olapDiscoverService.getNativeConnection(scube
					.getConnectionName());
	...
	public void closeQuery(String queryName) {
		try {
			...
			throw new SaikuServiceException(
					"Error closing query: " + queryName, e);
		}
	}
	...
	public void cancel(String queryName) {
		try {
			// System.out.println("Cancel: ID " + Thread.currentThread().getId()
			// + " Name: " + Thread.currentThread().getName());
			IQuery q = getIQuery(queryName);
			q.cancel();
		} catch (Exception e) {
			throw new SaikuServiceException("Error cancelling query: "
					+ queryName, e);
		}
	}
	...
	public CellDataSet execute(String queryName, ICellSetFormatter formatter) {
		try {
			...
			OlapConnection con = olapDiscoverService.getNativeConnection(query
					.getSaikuCube().getConnectionName());
			Long start = (new Date()).getTime();
			if (query.getScenario() != null) {
				log.info("Query (" + queryName + ") Setting scenario:"
			...
			log.info("Size: " + result.getWidth() + "/" + result.getHeight()
					+ "\tExecute:\t" + (exec - start) + "ms\tFormat:\t"
					+ (format - exec) + "ms\t Total: " + (format - start)
					+ "ms");
			...
			throw new SaikuServiceException(
					"Can't execute query: " + queryName, e);
		} catch (Error e) {
			throw new SaikuServiceException(
					"Can't execute query: " + queryName, e);
		}
	...
	public SaikuQuery simulateTag(String queryName, SaikuTag tag) {
		try {
			...
			throw new SaikuServiceException("Can't apply tag: " + tag
					+ " to query " + queryName, e);
		}
	}

	private IQuery applyTag(IQuery query, OlapConnection con, SaikuTag t)
			throws Exception {
		...
		if (t.getSaikuTupleDimensions() != null) {
			...
			if (t.getSaikuTupleDimensions().size() > 0) {
				SaikuTupleDimension rootDim = t.getSaikuTupleDimensions()
						.get(0);
				...
				for (SaikuTuple tuple : t.getSaikuTuples()) {
					SaikuMember m = tuple.getSaikuMember(rootDim
							.getUniqueName());
					List<SaikuMember> others = tuple
							.getOtherSaikuMembers(rootDim.getUniqueName());
					Selection sel = dim.createSelection(IdentifierParser
							.parseIdentifier(m.getUniqueName()));
					for (SaikuMember context : others) {
						QueryDimension otherDim = dimensionMap.get(context
								.getDimensionUniqueName());
						query.moveDimension(otherDim, Axis.COLUMNS);
						Selection ctxSel = otherDim
								.createSelection(IdentifierParser
		...
		if (t.getSaikuDimensionSelections() != null) {
			for (SaikuDimensionSelection dimsel : t
					.getSaikuDimensionSelections()) {
				if (!dimsel.getName().equals("Measures")) {
					QueryDimension filterDim = query.getDimension(dimsel
							.getName());
					...
					for (SaikuSelection ss : dimsel.getSelections()) {
						if (ss.getType() == SaikuSelection.Type.MEMBER) {
							Selection sel = filterDim
									.createSelection(IdentifierParser
											.parseIdentifier(ss.getUniqueName()));
							if (!filterDim.getInclusions().contains(sel)) {
								filterDim.getInclusions().add(sel);
							}
						}
					}
					// TODO: Move it to columns since drilling through with 2
					// filter items of the same dimension doesn't work
	...
	public ResultSet drillthrough(String queryName, int maxrows, String returns) {
		try {
			final OlapConnection con = olapDiscoverService
					.getNativeConnection(getQuery(queryName).getCube()
							.getConnectionName());
			...
			throw new SaikuServiceException("Error DRILLTHROUGH: " + queryName,
					e);
		}
	}

	public ResultSet drillthrough(String queryName, List<Integer> cellPosition,
			Integer maxrows, String returns) {
		try {
			IQuery query = getIQuery(queryName);
			CellSet cs = query.getCellset();
			SaikuCube cube = getQuery(queryName).getCube();
			final OlapConnection con = olapDiscoverService
					.getNativeConnection(cube.getConnectionName());
			final OlapStatement stmt = con.createStatement();
			...
			for (int i = 0; i < cellPosition.size(); i++) {
				List<Member> members = cs.getAxes().get(i).getPositions()
						.get(cellPosition.get(i)).getMembers();
			...
			SelectNode sn = (new DefaultMdxParserImpl()
					.parseSelect(getMDXQuery(queryName)));
			final Writer writer = new StringWriter();
			sn.getFilterAxis().unparse(
					new ParseTreeWriter(new PrintWriter(writer)));
	...
	public byte[] exportResultSetCsv(ResultSet rs, String delimiter,
			String enclosing, boolean printHeader,
			List<KeyValue<String, String>> additionalColumns) {
		return CsvExporter.exportCsv(rs, delimiter, enclosing, printHeader,
				additionalColumns);
	}

	public void setCellValue(String queryName, List<Integer> position,
			String value, String allocationPolicy) {
		try {
			...
			if (query.getScenario() == null) {
				...
				query.setScenario(s);
				con.setScenario(s);
				System.out.println("Created scenario:" + s + " : cell:"
						+ position + " value" + value);
			} else {
				s = query.getScenario();
				con.setScenario(s);
				System.out.println("Using scenario:" + s + " : cell:"
						+ position + " value" + value);
			...
			if (v == null) {
				throw new SaikuServiceException("Error setting value of query "
						+ queryName + " to:" + v);
			}
	...
	public boolean includeMember(String queryName, String dimensionName,
			String uniqueMemberName, String selectionType, int memberposition) {
		IQuery query = getIQuery(queryName);
		List<IdentifierSegment> memberList = IdentifierNode.parseIdentifier(
				uniqueMemberName).getSegmentList();
		QueryDimension dimension = query.getDimension(dimensionName);
		final Selection.Operator selectionMode = Selection.Operator
				.valueOf(selectionType);
		try {
			Selection sel = dimension
					.createSelection(selectionMode, memberList);
			...
			throw new SaikuServiceException("Cannot include member query ("
					+ queryName + ") dimension (" + dimensionName
					+ ") member (" + uniqueMemberName + ") operator ("
					+ selectionType + ") position " + memberposition, e);
		}
	}

	public boolean removeMember(String queryName, String dimensionName,
			String uniqueMemberName, String selectionType)
		...
		try {
			if (log.isDebugEnabled()) {
				log.debug("query: " + queryName + " remove:"
						+ selectionMode.toString() + " " + memberList.size());
			}
			Selection selection = dimension.createSelection(selectionMode,
					memberList);
			dimension.getInclusions().remove(selection);
			return true;
		} catch (OlapException e) {
			throw new SaikuServiceException("Error removing member ("
					+ uniqueMemberName + ") of dimension (" + dimensionName
					+ ")", e);
		}
	}

	public boolean includeLevel(String queryName, String dimensionName,
			String uniqueHierarchyName, String uniqueLevelName) {
		IQuery query = getIQuery(queryName);
		QueryDimension dimension = query.getDimension(dimensionName);
	...
	public boolean removeLevel(String queryName, String dimensionName,
			String uniqueHierarchyName, String uniqueLevelName) {
		IQuery query = getIQuery(queryName);
		QueryDimension dimension = query.getDimension(dimensionName);
		try {
			for (Hierarchy hierarchy : dimension.getDimension()
					.getHierarchies()) {
				if (hierarchy.getUniqueName().equals(uniqueHierarchyName)) {
					for (Level level : hierarchy.getLevels()) {
						if (level.getUniqueName().equals(uniqueLevelName)) {
							Selection inclusion = dimension
									.createSelection(level);
							...
							for (Selection sel : dimension.getInclusions()) {
								if ((sel.getRootElement() instanceof Member)) {
									if (((Member) sel.getRootElement())
										...
										if (dimension.getInclusions().contains(
												sel)) {
											removals.add(sel);
										}
									}
			...
			throw new SaikuServiceException("Cannot remove level"
					+ uniqueLevelName + "from dimension " + dimensionName, e);
		}
		return true;
	}

	public void moveDimension(String queryName, String axisName,
			String dimensionName, int position) {
		try {
			if (log.isDebugEnabled()) {
				log.debug("move query: " + queryName + " dimension "
						+ dimensionName + " to axis " + axisName + "  position"
						+ position);
			}
			IQuery query = getIQuery(queryName);
			QueryDimension dimension = query.getDimension(dimensionName);
			Axis newAxis = axisName != null ? ("UNUSED".equals(axisName) ? null
					: Axis.Standard.valueOf(axisName)) : null;
			if (position == -1) {
			...
			throw new SaikuServiceException("Cannot move dimension:"
					+ dimensionName + " to axis: " + axisName, e);
		}
	}

	public void removeDimension(String queryName, String axisName,
	...
	public List<SaikuDimensionSelection> getAxisSelection(String queryName,
			String axis) {
		IQuery query = getIQuery(queryName);
		...
		try {
			...
			throw new SaikuServiceException("Cannot get dimension selections",
					e);
		}
		return dimsel;
	}

	public SaikuDimensionSelection getAxisDimensionSelections(String queryName,
			String axis, String dimension) {
		IQuery query = getIQuery(queryName);
		try {
			QueryAxis qaxis = query.getAxis(axis);
			if (qaxis != null) {
				...
				if (dim != null) {
					...
					throw new SaikuOlapException(
							"Cannot find dimension with name:" + dimension);
				}
			} else {
				throw new SaikuOlapException("Cannot find axis with name:"
						+ axis);
			}
		} catch (SaikuOlapException e) {
			throw new SaikuServiceException("Cannot get dimension selections",
	...
	public void sortAxis(String queryName, String axisName, String sortLiteral,
			String sortOrder) {
		IQuery query = getIQuery(queryName);
		if (Axis.Standard.valueOf(axisName) != null) {
			QueryAxis qAxis = query.getAxis(Axis.Standard.valueOf(axisName));
	...
	public Properties getProperties(String queryName) {
		...
		try {
			...
			if (query.getDimension("Scenario") != null) {
				props.put("org.saiku.connection.scenario",
	...
	public byte[] getExport(String queryName, String type, String formatter,
		...
		if (formatter.equals("flat")) {
			...
			return getExport(queryName, type,
					new HierarchicalCellSetFormatter(), reportTitle);
		}
	...
	public byte[] getExport(String queryName, String type,
		...
		if (type != null) {
			...
			if (type.toLowerCase().equals("pdf")) {
				return PdfExporter.exportPdf(rs, formatter, reportTitle);
			}
	...
	public void qm2mdx(String queryName) {
		...
		OlapConnection con = olapDiscoverService.getNativeConnection(query
				.getSaikuCube().getConnectionName());
		MdxQuery mdx = new MdxQuery(con, query.getSaikuCube(), query.getName(),
				getMDXQuery(queryName));
		putIQuery(queryName, mdx);
		query = null;
	}

	public SaikuTag createTag(String queryName, String tagName,
			List<List<Integer>> cellPositions) {
		try {
			IQuery query = getIQuery(queryName);
			SaikuCube cube = getQuery(queryName).getCube();
			CellSet cs = query.getCellset();
			...
			for (List<Integer> cellPosition : cellPositions) {
				...
				for (int i = 0; i < cellPosition.size(); i++) {
					members.addAll(cs.getAxes().get(i).getPositions()
							.get(cellPosition.get(i)).getMembers());
				}
				...
				if (dimensions.size() == 0) {
					for (Member m : members) {
						SaikuTupleDimension sd = new SaikuTupleDimension(m
								.getDimension().getName(), m.getDimension()
								.getUniqueName(), m.getDimension().getCaption());
			...
			List<SaikuDimensionSelection> filterSelections = getAxisSelection(
					queryName, "FILTER");
			...
			throw new SaikuServiceException("Error addTag:" + tagName
	...
	// written by pradeep
	public byte[] exportDrillThroughResultSetPdf(ResultSet rs,
			String reportTitle) {
		return DrillThroughPdfExport.pdfExport(rs, reportTitle);
