...
public class GetLastTrxnsToBankProcessor extends BankRequestProcessor {
	...
	public ISOMsg process(CFIXMsg fixmsg) throws AllElementsNotAvailableException {
		...
		try {
			...
			isoMsg.set(2, mpan);// msg.getSourceCardPAN()
			...
			else if (CmFinoFIX.BankAccountType_CreditCard.toString().equals(msg.getSourceBankAccountType()))
			    processingCode = processingCode + constantFieldsMap.get("CREDITCARD_ACCOUNT");
			else{
				processingCode = CmFinoFIX.ISO8583_ProcessingCode_Sinarmas_Get_Last_Transactions;
			}*/
			
			isoMsg.set(3,CmFinoFIX.ISO8583_ProcessingCode_Sinarmas_Get_Last_Transactions);
