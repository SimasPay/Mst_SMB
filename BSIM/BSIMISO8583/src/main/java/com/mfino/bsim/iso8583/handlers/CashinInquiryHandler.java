...
public class CashinInquiryHandler extends FIXMessageHandler {

	private static Logger log = LoggerFactory.getLogger(CashinInquiryHandler.class);
	private SessionFactory sessionFactory;
	private HibernateTransactionManager htm;
	private static CashinInquiryHandler cashinInquiryHandler;
	private SubscriberMdnService subscriberMdnService; 
	private SubscriberService subscriberService;
	private TransactionLogService transactionLogService;

	public static CashinInquiryHandler createInstance(){
		if(cashinInquiryHandler==null){
			cashinInquiryHandler = new CashinInquiryHandler();
		}
		
		return cashinInquiryHandler;
	}
	
	public static CashinInquiryHandler getInstance(){
		if(cashinInquiryHandler==null){
	...
	public void handle(ISOMsg msg) throws Exception {
		...
		try{
			...
			log.info("CashinInquiryHandler :: handle()");
	...
	private void logTransactionRecord(ISOMsg msg, String accountNumber) {
		...
		CMCashInInquiry cashinquiry = new CMCashInInquiry();
		cashinquiry.setAmount(new BigDecimal(msg.getString("4")));
		cashinquiry.setDestMDN(accountNumber);
		cashinquiry.setOriginalReferenceID(new Long(msg.getString("11")));
		cashinquiry.setServiceName(ServiceAndTransactionConstants.TRANSACTION_CASHIN_INQUIRY);
		
		getTransactionLogService().saveTransactionsLog(CmFinoFIX.MessageType_CashInInquiry,cashinquiry.DumpFields());
	}
