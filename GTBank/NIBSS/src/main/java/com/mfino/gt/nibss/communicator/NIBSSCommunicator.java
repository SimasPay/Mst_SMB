...
import org.tempuri.NIPServiceSoap;
...
public abstract class NIBSSCommunicator extends BillPaymentsBaseServiceImpl{
	...
	private NIPServiceSoap nibssService;

	private Map<String, String> params;
	
	public NIPServiceSoap getNibssService() {
		return nibssService;
	}

	public void setNibssService(NIPServiceSoap nibssService) {
	...
	public MCEMessage process(MCEMessage mceMessage){
		...
		try
		{
			...
			if("MM_FundTransferSingleItem".equals(messageName)){
				returnXml = nibssService.mmFundTransferSingleItem(Long.valueOf((String)parameters.get(0)), (String)parameters.get(1), (String)parameters.get(2), (String)parameters.get(3), (BigDecimal)parameters.get(4), (String)parameters.get(5), "" + (Long)parameters.get(6), (String)parameters.get(7), (String)parameters.get(8));
			}
			else if("MM_BenNameEnquirySingleItem".equals(messageName)){
				returnXml = nibssService.mmBenNameEnquirySingleItem((String)parameters.get(0), (String)parameters.get(1));
			}
			else if("MM_TransactionQuery".equals(messageName)){
				returnXml = nibssService.mmTransactionQuery((String)parameters.get(0));
			}
			else if("MM_TransactionQuerySingleItem".equals(messageName)){
				returnXml = nibssService.mmTransactionQuerySingleItem((String)parameters.get(0), (String)parameters.get(1));
			}
