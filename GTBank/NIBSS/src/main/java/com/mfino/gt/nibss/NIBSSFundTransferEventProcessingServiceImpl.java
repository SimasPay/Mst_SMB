 ...
 *
 */
public class NIBSSFundTransferEventProcessingServiceImpl extends BillPayEventProcessingServiceImpl {
	...
	public MCEMessage processEvent(@Body MCEMessage mceMessage, @Header("eventContext") String eventContext) {
		log.info("IETEventProcessingServiceImpl : processEvent BEGIN eventContext=" + eventContext + "mceMessage="+mceMessage);
		...
		if(mceMessage.getResponse() instanceof BackendResponse){
			...
			if(BillPayConstants.SRC_SUSPENSE_INQ_FAILED.equals(eventContext)){
				...
				if(CmFinoFIX.ResponseCode_Success.equals(backendResponse.getResult())){
					backendResponse.setInternalErrorCode(NotificationCodes.InterEmoneyTransferInquirySuccessful.getInternalErrorCode());
				}
				else{
					backendResponse.setInternalErrorCode(NotificationCodes.InterEmoneyTransferFailed.getInternalErrorCode());
				}
			}
				...
				sendNotification = true;
				backendResponse.setInternalErrorCode(NotificationCodes.InterEmoneyTransferConfirmationSuccessful.getInternalErrorCode());
			}
...
//				backendResponse.setInternalErrorCode(NotificationCodes.BillpaymentConfirmationSuccessful.getInternalErrorCode());
	...
	public MCEMessage constructResponse(MCEMessage mceMessage){
		...
		if((null != billPaymentsRecord) && (null != sctl)){
			...
			response.setReceiverMDN(billPaymentsRecord.getInvoiceNumber()); // As invoice number contains the Receiver MDN
			SubscriberServiceImpl subscriberServiceImpl = new SubscriberServiceImpl();

			if(!(subscriberServiceImpl.normalizeMDN(sctl.getSourceMDN()).equals(subscriberServiceImpl.normalizeMDN(billPaymentsRecord.getInvoiceNumber())))){
			...
			response.setBeneficiaryName(response.getBeneficiaryName());
		}
