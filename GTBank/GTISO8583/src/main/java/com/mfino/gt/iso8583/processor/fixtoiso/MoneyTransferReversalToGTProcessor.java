...
public class MoneyTransferReversalToGTProcessor extends GTBankRequestProcessor {

	public MoneyTransferReversalToGTProcessor() {
		try {
			isoMsg.setMTI("0420");
		}
	...
	public ISOMsg process(CFIXMsg fixmsg) throws AllElementsNotAvailableException {
		...
		try {
			...
			if (CmFinoFIX.TransactionUICategory_Dompet_EMoney_Trf.equals(msg.getUICategory())) {
				isoMsg.set(41, GetConstantCodes.GT_DE41_BANK_SVA_TRF); // 41
				isoMsg.set(42, GetConstantCodes.GT_DE42_BANK_SVA_TRF); // 42
			}
			else {
				// Currently only two scenarions Bank->Emoney or Emoney->Bank
				isoMsg.set(41, GetConstantCodes.GT_DE41_SVA_BANK_TRF); // 41
				isoMsg.set(42, GetConstantCodes.GT_DE42_SVA_BANK_TRF); // 42
			}
			...
			try {
	            ...
	            originalMsg = MoneyTransferToGTProcessor.MTI
	            		+ // orginal message MTI
	            ...
	            /**
				 * Setting field 90 with information for enabling the reversal
				 */
			...
			isoMsg.set("127.0", GetConstantCodes.GT_DE127_0);
